 ...
 * A step is an implementation of the interface
 ...
 * is based on a predefined list of steps to be returned in the specified order
 * when a step is needed.
 * <p>
 * A wrap is class implementing an interface in terms of another object
 * implementing the same interface (the wrapped object).
 * Each method in the wrap invokes the next step from the program associated
 * with the wrap, passing a functional calling the corresponding method in
 * the wrapped object.
 * For example
 ...
 * <ul>
 *  <li>When the driver is loaded, it logs its version with level {@link java.util.logging.Level#CONFIG}.</li>
